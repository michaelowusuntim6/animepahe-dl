...
set_var() {
    ...
    _PYTHON="$(command -v python3)" || command_not_found "python3"
    ...
    # --- Linux / Pop!_OS download directory ---
    _DOWNLOAD_DIR="$HOME/Videos/Anime"
    mkdir -p "$_DOWNLOAD_DIR"

    # --- Concurrent fragments for yt-dlp (default 16) ---
...
download_episode() {
    ...
    if [[ -z ${_LIST_LINK_ONLY:-} ]]; then
        ...
        # Pass the cf_clearance cookie to yt-dlp so all CDN requests carry it,
        # otherwise the CDN (e.g. vault-*.uwucdn.top) can return 403 Forbidden.
        # Do NOT force --downloader ffmpeg here: those CDNs require HTTP/2
        # (HTTP/1.1 requests are 403'd), and ffmpeg's downloader is
        # HTTP/1.1-only. yt-dlp's native HLS downloader uses curl_cffi with
        # HTTP/2 + Chrome impersonation; pycryptodomex handles AES-128
        # decryption natively, so ffmpeg is only used afterwards to remux the
        # finished file locally into MP4.
        local cookie_header="Cookie: cf_clearance=$_CF_CLEARANCE"
        "$_YTDLP" "$pl" \
        ...
        # yt-dlp reports this HLS stream's container as mp4 even though the
        # segments are MPEG-TS, so its own remux postprocessor is skipped.
        # Remux locally with a stream copy (fast, no re-encode) into a real
        # MP4 container.
